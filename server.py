...
class hmac():
    def POST(self):
        data = web.input()
        valid = self.validate_sig(data.file, data.sig)
        if valid:
    ...
    def validate_sig(self, file, sig):
        key = 'YELLOW SUBMARINE'
        file = file.encode('ascii')
        computed_sig = mac.hmac_sha1(key, file)
        return self.insecure_compare(sig, computed_sig)

    def insecure_compare(self, sig1, sig2):
